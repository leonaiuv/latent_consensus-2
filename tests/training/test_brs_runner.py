...
def test_run_brs_experiment_real_mode_supports_evaluate_only_checkpoint(
    monkeypatch,
    tmp_path,
) -> None:
    from latent_consensus.models.latent_consensus_causal_lm import LatentConsensusCausalLM
    ...
    output_root = tmp_path / "results"
    initial_result = run_brs_experiment(
        experiment_id="EXP-B03",
        configs_dir=ROOT / "configs",
    ...
    checkpoint_path = output_root / "EXP-B03" / "best_checkpoint.pt"
    assert checkpoint_path.is_file()
    assert initial_result["runtime_mode"] == "real"

    def fail_if_fit_called(self, train_dataset, val_dataset):
        raise AssertionError("evaluate-only 模式不应再次调用 fit")

    monkeypatch.setattr(
        "latent_consensus.training.causal_lm_trainer.CausalLMTrainer.fit",
        fail_if_fit_called,
    )
    ...
    result = run_brs_experiment(
        ...
        ood_limit_per_step=1,
        seq_len=64,
        evaluate_only_checkpoint_path=checkpoint_path,
    )

    experiment_dir = output_root / "EXP-B03"
    assert result["history"]["best_epoch"] == 0
    assert (experiment_dir / "test_metrics.json").is_file()
    assert (experiment_dir / "ood_metrics.json").is_file()
    assert (experiment_dir / "summary.json").is_file()
